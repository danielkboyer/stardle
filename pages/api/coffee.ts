...
//console.log(coffeeRes);
  ...
  var names = "";
  coffeeRes.data.forEach((element: { support_coffees:string;supporter_name: string; support_coffee_price: string }) => {
  
      
    
    const price = parseFloat(element.support_coffee_price);
    const amountBought = parseFloat(element.support_coffees);
    names+="|"+element.supporter_name+": $"+Math.round(price*amountBought)+"|";
    names +=" ";
